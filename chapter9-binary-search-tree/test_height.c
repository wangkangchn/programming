 ...
 * @return: 新分配的节点指针
 */
Node* calloc_node(int key, Node *parent)
{
...
/**
 * is_left - 确定节点是父节点的左子树还是右子树
 * @node:	待检测树节点
 * @return: 1 左子树
 *          0 右子树
 */
#define is_left(node) ({    (node)->parent->left == (node);  })
...
/* LL型 右旋 
  ...
  /3
*/
static inline void LL(Node **node)  
{   
...
/* RR型 左旋
    \1 (parent)         2(parent)
     \2 (node)   (node)1  3    
      \3
 */
static inline void RR(Node **node)
{
...
/* LR型 先左旋后右旋 
      ...
      2 (node)  =>     3 (node)  =>     3
       \              /                / \
        3            2                2   1
*/
static inline void LR(Node **node)
{
    RR( &((*node)->left) );   
    LL( node );     
}  
...
/* RL型 先右旋后左旋 
    ...
    3                   2            1    2
*/
static inline void RL(Node **node)
{   
    LL( &((*node)->right) );  
    RR( node );     
} 
 ...
 * @return: 无
 */
void insert(Node **tree, int key, Node *root)
{
    /* 来到所需的叶节点, 插入 */
    if (*tree == NULL) {
        pr_debug("插入: %d\n", key);
        Node *node = calloc_node(key, *tree);
        *tree = node;
        if (root == NULL) {
            root = node;
            node->parent = root;
    ...
    /* 向左子树插 */
    if (key <= (*tree)->key)
        insert(&(*tree)->left, key, root);
    /* 向右子树插 */
    else
        insert(&(*tree)->right, key, root);
...
int main(int argc, char *argv[])
{
    ...
    printf("中序遍历\n");
    traversal(bs_tree, IN_ORDER, show);
